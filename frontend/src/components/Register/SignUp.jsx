      ...
      fetch('http://localhost:5005/admin/auth/register', {
        ...
        .then(data0 => {
          if (data0.error) {
            ...
            localStorage.setItem('token', data0.token);
            apiCallPut('store', data0.token, { store: { email: data.get('email'), userName: data.get('firstName') + ' ' + data.get('lastName'), presentations: [] } });
            console.log(data0);
            navigate('/presto');
          }
        })
